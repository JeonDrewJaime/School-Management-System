...
export default function Login() {
  ...
  const FORM_VALIDATION = Yup.object().shape({
    ...
    .test('len', 'Must be exactly 11 numbers.', val => !val || (val && val.toString().length === 1))
    .integer()
    .typeError('Only enter a number.')
    .required('This field is required.'),
     student_pass: Yup.string()
    .required('This field is required.')
  });
  useEffect(() => {
    setLoading(true)
              ...
              console.log(error);
                ...
                if (response && response.status == 422) {
                  ...
                  if(response.data.errors) {
                    ...
                    setErrors({
                      student_id:[response.data.message]
                    })
                  }
                }
            });
          ...
          <Grid item xs={5} sx={{ mt: '5.5vmin', alignSelf: 'center'}} > 
            <Formik initialValues={{...INITIAL_FORM_STATE}} validationSchema={FORM_VALIDATION} onSubmit={handleSubmit} >  
              ...
              <Grid container rowSpacing={4} rowGap = {1} method = "post" >
                <Grid item xs = {12}  >    
                <TextFields  animation = "fade-in-left" label = "User ID"  id = "studentID" bgc = "white" fullW = {true} name = "student_id"/>
                </Grid>
                <Grid item xs = {12}> 
                  <TextFields type = "password" animation = "fade-in-left" label = "Password" bgc = "white" fullW = {true} name = "student_pass"/>
                </Grid>
                <Grid item xs = {12}> 
